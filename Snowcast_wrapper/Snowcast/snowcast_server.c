...
void init_server_locl(void);
void init_clients_info(void);
int find_first_client(void);
        ...
        while (loop != 16)
        {
            ...
            else if (ret == 0)
            {
                ...
                for (clientinfo_t* start = station->connected_clients.head_clients;
                     start != NULL;
                     start = start->next_client)
                {
                    
                    assert(start->state == HANDSHAKED);
                    assert(start->udp_sock != 0);
                    
                    if ((num_bytes = sendto(start->udp_sock,
                                            buf_ptr,
                                            ret,
                                            0,
...
void remove_client_from_station(clientinfo_t* client)
{
    ...
    if (client == c)
    {
        ...
        station->connected_clients.head_clients =
            station->connected_clients.head_clients->next_client;
        
        if (station->connected_clients.tail_clients == client) // only one item
        {
            station->connected_clients.tail_clients =
                station->connected_clients.head_clients;
                ...
                else
				{
					...
					if ((nbytes = read_line(i, buf, BUF_SIZE - 1)) <= 0)
					{
						...
						if (nbytes == 0)
						{
                            ...
                            strcpy(buf_ptr, "STOP");
                            int num_bytes = 0;
                            if ((num_bytes = sendto(cl->udp_sock,
                                                    buf_ptr,
                                                    4,
                                                    0,
