/**
 * File: DropboxFileServerSyncer.java
...
class DropboxFileServerSyncer extends ThreadBase{
	 ...
	 */
	public DropboxFileServerSyncer(Socket sock,
								   DropboxFileServer server,
								   String dir){
		...
		try{
			...
			_sp = new SyncStreamParser(_fm.getHome(),
				  new DataInputStream(_sock.getInputStream()),
				  					  _server.debugMode());
			_sw = new SyncStreamWriter(_fm.getHome(),
				  new DataOutputStream(_sock.getOutputStream()),
				  					   _server.debugMode());
