...
final class DropboxFileServer {
	...
	private int _prio; // The priority of this file server. (1->5)
	               	   // The larger it is the higher
					   // it will be chosen to be the sync server
	 ...
	 * @param maxClientNum: the max client number
	 * @param port: the port number
	 * @param debug: debug mode?
	 * @param useUI: use UI?
	 * @param hideException: hide Exception?
	 * @param disk: disk path?
	 */
    public DropboxFileServer(int id,
    						 int prio,
    						 int maxClientNum,
    						 int port,
    						 boolean debug,
    ...
    private void initTimer(){
    	...
    	_timer = new Timer();
    	_timer.schedule(new Echo(this),
    					DropboxConstants.ECHO_FILE_SERVER,
    					DropboxConstants.ECHO_FILE_SERVER);
    }
    ...
    /**
     * run: run the whole story of file server, firstly connect to master
     *      and then start to listen to clients
     */
    public void run(){
    ...
    /**
     * cancelTimer: cancel the timer thread
     */
    ...
    /**
     * cancelListeningThread: basically just cancel the network thread
     */
    ...
    /**
     * cancelSyncers: cancel all of the syncer threads
     */
    ...
    /**
     * findMatch: find if there is a client node has the given name and password
     * @param name: the name of the client
     * @param password: the password of the client
     * @return: the client node, or null if cannot find 
     */
    ...
    /**
     * appendClientToFile: append a single item of client to file
     * @param name: the name of the client
     * @param password: the password of the client
     */
    public void appendClientToFile(String name, String password){
    	
    	File file = new File(_disk +
		...
		try{
			...
			FileWriter fw = new FileWriter(_disk +
										   System.getProperty("file.separator") +
										   "clients.txt", true);
    ...
    /**
     * writeClientsToFile: write the client information into the file
     */
    public void writeClientsToFile(){
    	
    	File file = new File(_disk + System.getProperty("file.separator") +
    						 "clients.txt");
		try{
			// if file doesnt exists, then create it
			if (!file.exists()) {
    ...
    /**
     * loadClientsFromFile: load clients' information from file
     */
    ...
    public boolean changePassword(String name, String oldPassword,
    							  String password){
    ...
    public boolean addClient(String clientName, String password,
    	...
    	String fullpath = _disk +  System.getProperty("file.separator") +
    					  clientName;
    ...
    /**
     * garbageCollection: collect dead syncers
     */
    ...
    /**
     * Class: Echo
     * Description: this class handles periodical functionalities
     */
    private class Echo extends TimerTask{
		...
		/**
		 * Constructor
		 * @param server: the file server object
		 */
		...
		/**
		 * run: periodically run garbage collection and dump debug info
		 */
		@Override
		public void run(){
    ...
    public static void main(String[] args) {
    	...
    	for( int i = 0; i < args.length; i++ ){
    		...
    		if (args[i].equals("-d"))
				...
				maxClientNum = Math.min(maxClientNum,
										DropboxConstants.MAX_CLIENTS_IN_FS);
			}
    	...
    	DropboxFileServer server = new DropboxFileServer(id,
    													 prio,
    													 maxClientNum,
    													 port,
    													 debug,
