/**
 * File: MasterServerClusterNet.java
 ...
 * Description: listen to new file server connected and
...
class MasterServerClusterNet extends GeneralServer {
	...
	/**
	 * parse: parse the string and send response to the socket
		...
		if(tk.equals(ProtocolConstants.PACK_STR_ID_HEAD)){
			...
			try
			{
				...
				while(st.hasMoreTokens()){
					...
					boolean result = _server.addClient(clientName,
													   password,
													   node);
					if(result == false){
						NetComm.send(ProtocolConstants.PACK_STR_ERRMES_HEAD + " "
								+ "Conflict in adding client,"
								+ " the client name already exists!", out);
						out.close();
						node = null;
				...
				MasterServerFileServerRequest fsq =
					new MasterServerFileServerRequest(_server, out, in, s, node);
			...
			try
			{
				...
				MasterServerFileServerAccept fsa =
					new MasterServerFileServerAccept(_server, out, in, s, node);
	...
	/**
	 * run: start to listen the new file server and parse the first
	 * 	    stream got from the socket
	 */
	@Override
	public void run(){
		...
		try{
			...
			while(true)
			{
				...
				_log("Get connection from " +
					 fileServer.getInetAddress().getHostAddress());
				//TODO: here, it should control how many
				//      threads we could accept at most

				// read the identification
				BufferedReader in = new BufferedReader(
						    new InputStreamReader(fileServer.getInputStream()));
